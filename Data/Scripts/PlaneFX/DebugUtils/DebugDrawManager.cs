...
namespace PlaneFX.DebugUtils
{
    ...
    public class DebugDraw : MySessionComponentBase
    {
        ...
        public static void DrawLineZT(Vector3D origin, Vector3D destination, Color color, float width = 0.5f)
        {
            Instance?.DrawLine0(origin, destination, color, width);
        }

        private void DrawLine0(Vector3D origin, Vector3D destination, Color color, float width = 0.5f)
        {
            float length = (float)(destination - origin).Length();
            Vector3D direction = (destination - origin) / length;

            MyTransparentGeometry.AddLineBillboard(MaterialSquare, color, origin, direction, length, width, blendType: BlendTypeEnum.PostPP);

            float depthScale = ToAlwaysOnTop(ref origin);
            direction *= depthScale;

            MyTransparentGeometry.AddLineBillboard(MaterialSquare, color * OnTopColorMul, origin, direction, length, width * depthScale, blendType: BlendTypeEnum.PostPP);
        }
